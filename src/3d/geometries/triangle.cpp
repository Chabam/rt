...
constexpr std::array<glm::vec2, Triangle::POINT_COUNT> default_uvs = {glm::vec2{0.5f, 1.f}, glm::vec2{1.f, 0.f},
                                                                      glm::vec2{0.f, 0.f}};
Triangle::Triangle(const std::array<glm::vec3, POINT_COUNT>& pts)
    : m_vertices{}
    , m_indices{0, 1, 2}
    , m_normal{glm::normalize(glm::cross(pts[2] - pts[1], pts[0] - pts[1]))}
    , m_tangent{pts[2] - pts[1]}
    , m_bi_tangent{pts[0] - pts[1]}
{
    // glm::vec3 edge_1 = pts[1] - pts[0];
    // glm::vec3 edge_2 = pts[2] - pts[0];
    // glm::vec2 delta_uv_1 = default_uvs[1] - default_uvs[0];
    // glm::vec2 delta_uv_2 = default_uvs[2] - default_uvs[1];
